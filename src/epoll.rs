...
impl Poller {
    ...
    pub fn wait(&self, events: &mut Events, timeout: Option<Duration>) -> io::Result<()> {
        ...
        log::debug!("wait: waiting epoll_fd={}, timeout={:?} timeout_ms={}, events.list.len={}", self.epoll_fd, timeout, timeout_ms, events_list_len);
        ...
        log::trace!("wait: call modify to clear notification epoll_fd={}, res={}", self.epoll_fd, res);
        let mut buf = [0u8; 8];
        ...
        log::trace!("wait: retf modify to clear notification epoll_fd={}, res_rd={:?} buf={:?}", self.epoll_fd, res_rd, buf);

        // Re-register interest in it.
        log::trace!("wait: re-register epoll_fd={}", self.epoll_fd);
        self.modify(
            self.event_fd,
            Event {
                key: crate::NOTIFY_KEY,
    ...
    pub fn notify(&self) -> io::Result<()> {
        log::trace!("notify:+ epoll_fd={}, event_fd={}", self.epoll_fd, self.event_fd);
